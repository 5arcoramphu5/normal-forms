...
void multiplyAndAdd(Polynomial<Coeff> &accumulator, int accumulatorIndex, const Polynomial<Coeff> &p1, int p1Index, const Polynomial<Coeff> &p2, int p2Index)
{
    ...
    {
        capd::Multiindex index1({deg1, 0, 0, 0});
        do
        {
            for(int deg2 = 0; deg2 <= p2.degree(); ++deg2)
            {
                capd::Multiindex index2({deg2, 0, 0, 0});
                do
                {
                    capd::Multiindex indexSum({index1[0]+index2[0], index1[1]+index2[1], index1[2]+index2[2], index1[3]+index2[3]});
...
{
    if(jet.dimension() != N) throw std::runtime_error("invalid dimension for polynomial matrix multiplication.");
